...
class QuaternionTransposeConv(Module):
    ...
    def reset_parameters(self):
        ...
        self.r_weight.data.normal_(0, math.sqrt(1. / self.in_channels))
        self.i_weight.data.normal_(0, math.sqrt(1. / self.in_channels))
        self.j_weight.data.normal_(0, math.sqrt(1. / self.in_channels))
        self.k_weight.data.normal_(0, math.sqrt(1. / self.in_channels))
...
class QuaternionConv(Module):
    ...
    def reset_parameters(self):
        affect_init_conv(self.r_weight, self.i_weight, self.j_weight, self.k_weight,
                    self.kernel_size, self.winit, self.rng, self.init_criterion)
        #torch.nn.init.xavier_uniform(self.r_weight.data)
        
        #torch.nn.init.xavier_uniform(self.i_weight.data)
        #torch.nn.init.xavier_uniform(self.j_weight.data)
        #torch.nn.init.xavier_uniform(self.k_weight.data)
...
class QuaternionAttnConv(Module):
    r"""Applies a Quaternion Convolution to the incoming data.
    """
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
        ...
        super(QuaternionAttnConv, self).__init__()
