...
class AxialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Transformations
        #print("Input dimensions before call: ",x.shape)
    ...
    #def reset_parameters(self):
        #print("In reset: ",self.qkv_transform.weight.shape)
...
class AxialBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv_down = QuaternionConv(inplanes, width, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        identity = x  # print("Input: ",x.shape)  #10 32 56 56
        out = self.conv_down(x)
        out = self.bn1(out)
        out = self.relu(out)  #print("After Conv-down: ",out.shape) #10 64 56 56
	
        out = self.hight_block(out) #print("After Height-axis: ",out.shape)  #10 64 56 56
        out = self.width_block(out)
        out = self.relu(out)  #print("After width-axis: ",out.shape) #10 64 56 56

        out = self.conv_up(out)
        out = self.bn2(out) #print("After Conv-up: ",out.shape) #10 128 56 56
        if self.downsample is not None:
            identity = self.downsample(x) #print("After identity: ",identity.shape) #10 128 56 56
        out += identity #print("After Identity-sum: ",out.shape) #10 128 56 56
        out = self.relu(out) #print("Output before return: ",out.shape) #10 128 56 56
        return out
...
class AxialAttentionNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        #x = self.maxpool(x) #print("After Stem: ",x.shape) #10 32 56 56
	
        x = self.layer1(x) #print("After layer1: ",x.shape)   # 10 128 56 56
        x = self.layer2(x) #print("After layer2: ",x.shape)   # 10 256 28 28
        x = self.layer3(x) #print("After layer3: ",x.shape)   # 10 512 14 14
        x = self.layer4(x) #print("After layer4: ",x.shape)   #10 1024 7 7
	
        x = self.avgpool(x)
        #print(x.shape)
        x = torch.flatten(x, 1)
        #print(x.shape)
        x = self.fc(x) #print("Before return: ",x.shape) #10 1000
        return x
